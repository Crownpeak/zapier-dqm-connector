...
describe('creates.getSpellcheckIssues', () => {
  ...
  const mockResponse = {
    "supportedLanguagesDetected": [],
    "unsupportedTextByLanguage": {},
    "misspellings": [],
    "created": 1758024862609
  }
  ...
  it('should return spellcheck issues for given asset', async () => {
    nock(baseUrl)
        .get(`/assets/${assetId}/spellcheck`)
    ...
    const result = await appTester(App.creates.getSpellcheckIssues.operation.perform, bundle);
  ...
  it('should throw MissingField when assetId is not provided', async () => {
    ...
    await expect(
        appTester(App.creates.getSpellcheckIssues.operation.perform, bundle)
  ...
  it('should return 404 for invalid assetId', async () => {
    ...
    nock(baseUrl)
        .get(`/assets/${invalidAssetId}/spellcheck`)
        .query({ apiKey, websiteId })
        .matchHeader('Accept', 'application/json; charset=UTF-8')
    ...
    try {
      await appTester(App.creates.getSpellcheckIssues.operation.perform, bundle);
    } catch (error) {
